...
processCmd(Lst, [2,A1|T], Step) ->
  io:fwrite("element ~p is at position ~p~n", [A1, indexOf(A1, Lst, 1)]), processCmd(Lst, T, Step - 1);

processCmd(Lst, [3,A1|T], Step) ->
  io:fwrite("element at position ~p is ~p~n", [A1, lists:nth(A1, Lst)]), processCmd(Lst, T, Step - 1);
